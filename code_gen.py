...
def search_in_symbol_table(item, scope_num=0):
    for record in need.symbol_table['ids'][::-1]:
...
class CodeGenerator:
    ...
    def call_routine(self, name, lookahead, input):
        print(name[1:])
    ...
    def define_variable(self, lookahead):
        ...
        if 'arr' not in var_id:
            need.symbol_table['ids'].append((var_id, 'int', address, self.current_scope))
        else:
            if "size" not in var_id:
                need.symbol_table['ids'].append((var_id, 'int*', address, self.current_scope))
            else:
                need.symbol_table['ids'].append((var_id, 'int', address, self.current_scope))
        print(need.symbol_table)
    ...
    def define_array(self, lookahead):
        ...
        need.symbol_table['ids'].append((array_id, 'int*', address, self.current_scope))
        print(need.symbol_table)
    ...
    def multiply(self, lookahead):
        ...
        self.type_mismatch(lookahead, self.SS[-1], self.SS[-2])
        self.SS.pop()
        self.SS.pop()
        self.SS.append(result_address)

    def define_array_argument(self, lookahead):
        temp = need.symbol_table['ids'][-1]
        print(temp)
        del need.symbol_table['ids'][-1]
        if temp[1] == 'int':
            need.symbol_table['ids'].append((temp[0], 'int', temp[2], temp[3]))
        else:
            need.symbol_table['ids'].append((temp[0], 'int*', temp[2], temp[3]))
        print(need.symbol_table)
    ...
    def repeat_jumps(self, lookahead):
    ...
    def finish_function(self, lookahead):
        ...
        for item in need.symbol_table['ids'][::-1]:
            if item[1] == 'function':
                if item[0] == 'main':
    ...
    def create_record(self, lookahead):
        ...
        func_id = self.SS[-3]
        args_start_idx = need.symbol_table['ids'].index('>>')
        func_args = need.symbol_table['ids'][args_start_idx + 1:]
        need.symbol_table['ids'].pop(args_start_idx)
        need.symbol_table['ids'] \
    ...
    def scope_check(self, lookahead):
        if search_in_symbol_table(lookahead[2], self.current_scope) or lookahead[2] == 'output':
            return
        need.semantic_errors.append(f'#{lookahead[0]}: Semantic Error! \'{lookahead[2]}\' is not defined.')

    def void_check(self, var_id):
        if self.id_type[2] == 'void':
            need.semantic_errors.append(f'#{self.id_type[0]}: Semantic Error! Illegal type of void for \'{var_id}\'.')

    def break_check(self, lookahead):
        if len(self.break_stack) > 0 and ['>>>' in self.break_stack]:
            return
        need.semantic_errors.append(
            f'#{lookahead[0] - 1}: Semantic Error! No \'repeat ... until\' found for \'break\'.')
    ...
    def type_mismatch(self, lookahead, operand_1, operand_2):
        ...
        if not operand_1.startswith('#'):
            for s in need.symbol_table['ids']:
        ...
        if operand_2_type != operand_1_type:
            ...
            print("meowwwwwwwwwwwwwwwwwwwwwwwwwwwwww", end=" ")
            print(
                f'#{lookahead[0]}: Semantic Error! Type mismatch in operands, Got {operand_2_type} instead of {operand_1_type}.')

    def parameter_num_matching(self, lookahead, args, attributes):
        func_name = ''
        for i in need.symbol_table['ids']:
            if i[2] == attributes:
                func_name = i[0]
        func_args = []
        for i in attributes:
    ...
    def parameter_type_matching(self, lookahead, var, arg, num):
        if arg.startswith('#'):
            ...
            for rec in need.symbol_table['ids']:
